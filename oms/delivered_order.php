...
<head>
  ...
  <title>CRM | Delivered  Order</title>
  ...
  <div class="content-wrapper">
	...
	<?php
		...
		// {
			...
			$ParamArray=array();
			$ParamArray[0]="Delivered";
			$Fields=implode(",",$FieldNames);
			$all_orders=$obj->MysqliSelect1("Select ".$Fields." from order_master where OrderStatus = ? ",$FieldNames,"s",$ParamArray);
			
			$OrderId=$all_orders[0]["OrderId"];
			$OrderDate=$all_orders[0]["OrderDate"];

            // Get the current month and year
            $CurrentMonth = date("m");
            $CurrentYear = date("Y");

            // Loop through and filter records
            ...
            foreach ($all_orders as $order) {
                ...
                if ($orderYear == $CurrentYear && $orderMonth == $CurrentMonth) {
                    $filtered_orders[] = $order; // Store matching records
                }
            }
