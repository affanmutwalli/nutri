...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Today's Order</h1>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
                                <li class="breadcrumb-item active">Today's Order</li>
                            </ol>
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
		...
		// Get today's shipped orders by default
		$FieldNames=array("Id", "OrderId", "CustomerId", "OrderDate", "Amount","TransactionId", "PaymentStatus", "OrderStatus", "ShipAddress", "PaymentType", "Waybill");
		$ParamArray=array();
		$Fields=implode(",",$FieldNames);

		if(isset($_GET) && array_key_exists("OrderDate",$_GET))
		{
			// If specific date is requested
			$ParamArray[0]=$_GET["OrderDate"];
			$single_data=$obj->MysqliSelect1("Select ".$Fields." from order_master where OrderDate= ? ",$FieldNames,"s",$ParamArray);
		}
		else
		{
			// Show today's shipped orders by default
			$single_data=$obj->MysqliSelect1("Select ".$Fields." from order_master where OrderStatus = 'Shipped' AND DATE(CreatedAt) = CURDATE() ORDER BY CreatedAt DESC",$FieldNames,"",array());
		}

		if(!empty($single_data) && isset($single_data[0])) {
			$OrderId=$single_data[0]["OrderId"];
			$OrderDate=$single_data[0]["OrderDate"];
		}
	  ?>
            <!-- Main content List-->
            <section class="content">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Today's Order List</h3>
                    </div>
                    <div class="card-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($single_data as $order) : ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($order['OrderId']); ?></td>
                                    <td><?php echo date("d-m-Y", strtotime($order['OrderDate'])); ?></td>
                                    <td><?php echo number_format($order['Amount'], 2); ?></td>
                                    <td>
                                        <?php if (!empty($order['Waybill'])) : ?>
                                            <a href="https://www.delhivery.com/track/package/<?php echo $order['Waybill']; ?>" target="_blank" class="btn btn-sm btn-info">
                                                <?php echo htmlspecialchars($order['Waybill']); ?>
                                            </a>
                                        <?php else : ?>
                                            <span class="badge badge-warning">No Waybill</span>
                                        <?php endif; ?>
                                    </td>
                                ...
                                <?php else : ?>
                                <tr>
                                    <td colspan="9" class="text-center">No shipped orders found for today.</td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
