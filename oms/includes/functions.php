...
function sec_session_start() {
    ...
    // Try to set ini_set, but don't fail if it's disabled
...
function change_user_password($email, $current_password_hash, $new_password_hash, $mysqli) {
    ...
    if ($update_stmt->execute()) {
        error_log("OMS Password changed successfully for user: " . $email);
        return array('success' => true, 'message' => 'Password changed successfully');
    } else {
        error_log("OMS Password change failed for user: " . $email . " - " . $mysqli->error);
